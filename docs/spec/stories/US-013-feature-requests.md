...
| D5 | The sticky add row opens `request-sheet.tsx`, a bottom sheet with a name field and a large details textarea, instead of an inline title input with a "+ details" toggle; rows truncate their title to one line and their note to two | Feedback after building: the inline note input was too small for anything but a one-liner. `RequestSheet` is the single add sheet for the whole feature — US-014's radial menu quick-capture uses it too (US-014 D7) rather than a second, differently-shaped sheet. Long notes need truncation in the list once they can run to the full 500 chars. |
