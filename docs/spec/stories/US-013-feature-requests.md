...
| D5 | The sticky add row opens a bottom sheet (name + a large details textarea) instead of an inline title input with a "+ details" toggle; rows truncate their title to one line and their note to two | Feedback after building: the inline note input was too small for anything but a one-liner, and the sheet matches the shape `QuickCaptureSheet` (US-014) already uses from the radial menu. Long notes need truncation in the list once they can run to the full 500 chars. |
