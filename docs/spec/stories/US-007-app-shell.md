...
| D5 | The list entrance is a CSS keyframe, not a Motion variant | Motion's `initial={{ opacity: 0 }}` ships `opacity: 0` in the SSR HTML, so slow or failed hydration leaves the content invisible — and the changelog accordions are required to work with no JS at all (US-006 AC 4). |
