...
| D4 | ~~No "add exercise mid-session"~~ **Reversed after owner feedback.** Skip and replace are still not built | The original reasoning — "add it to the plan day instead" — ignored ad-hoc sessions, which have no plan day. And because the seeded "My week" is seven rest days, "Start session" on a fresh install always created exactly that: a session with no exercises and no way to add one, so nothing could be logged. Added exercises persist in `gym_sessions.added_workout_ids` (migration `20260913120644`) and render after the plan's own, in the order added. |
| D4a | `20260913120418_session_added_exercises.sql` is **empty** | A scripting error pushed it to hosted before its SQL was written. Migrations applied to hosted are never rewritten (docs/setup.md), so it stays as an applied no-op and the change is in `20260913120644_session_added_workout_ids.sql`. Migration scripts now refuse to push an empty file. |
